! function(t, e) {
    ...
    return function(t) {
        ...
        1: function(t, e, i) {
            ...
            r.register = (t, e) => r[t] = t => new e(t), r.version = "0.5.9";
            const h = function() {
            ...
            r.VantaBase = class {
                constructor(t = {}) {
                    if (!o) return !1;
                    if (r.current = this, this.windowMouseMoveWrapper = this.windowMouseMoveWrapper.bind(this), this.windowTouchWrapper = this.windowTouchWrapper.bind(this), this.resize = this.resize.bind(this), this.animationLoop = this.animationLoop.bind(this), this.restart = this.restart.bind(this), this.options = Object(s.c)({
                            mouseControls: !0,
                            touchControls: !0,
                            minHeight: 200,
                            minWidth: 200,
                            scale: 1,
                            scaleMobile: 1
                        }, this.defaultOptions), (t instanceof HTMLElement || "string" == typeof t) && (t = {
                            el: t
                    ...
                    e("resize", this.resize), this.resize(), this.animationLoop(), this.options.mouseControls && (e("scroll", this.windowMouseMoveWrapper), e("mousemove", this.windowMouseMoveWrapper)), this.options.touchControls && (e("touchstart", this.windowTouchWrapper), e("touchmove", this.windowTouchWrapper))
                }
                setOptions(t = {}) {
            ...
            class h extends s.b {
                ...
                genPoint(t, e, i) {
                    ...
                    if (this.points || (this.points = []), this.options.showDots) {
                        const t = new r.SphereGeometry(.25, 12, 12),
                            e = new r.MeshLambertMaterial({
                ...
                onInit() {
                    ...
                    h.addAttribute("position", new r.BufferAttribute(this.linePositions, 3).setDynamic(!0)), h.addAttribute("color", new r.BufferAttribute(this.lineColors, 3).setDynamic(!0)), h.computeBoundingSphere(), h.setDrawRange(0, 0);
                    ...
                    this.camera = new r.PerspectiveCamera(20, this.width / this.height, .01, 1e4), this.camera.position.set(50, 100, 150), this.scene.add(this.camera);
                    const c = new r.AmbientLight(16777215, .75);
                    this.scene.add(c), this.spot = new r.SpotLight(16777215, 1), this.spot.position.set(0, 200, 0), this.spot.distance = 400, this.spot.target = this.cont, this.scene.add(this.spot), this.cont2 = new r.Group, this.cont2.position.set(0, 15, 0), this.scene.add(this.cont2);
                    const l = new r.LineBasicMaterial({
                    ...
                    for (let t = 0; t < 80; t++) {
                        const t = Object(o.h)(18, 24),
                            ...
                            h = Math.sin(n) * s,
                            a = Math.cos(n) * s;
                        p.vertices.push(new r.Vector3(a * t, h * t, i * t)), p.vertices.push(new r.Vector3(a * e, h * e, i * e))
                    }
                    this.linesMesh2 = new r.LineSegments(p, l), this.linesMesh2.position.set(0, 0, 0), this.cont2.add(this.linesMesh2);
                    ...
                    for (let t = 0; t < 4; t++) {
                        let e = .15 * Math.cos(t / 4 * Math.PI * 2),
                            i = .15 * Math.sin(t / 4 * Math.PI * 2),
                            s = [17.9, 12, 8, 5, 3, 2, 1.5, 1.1, .8, .6, .45, .3, .2, .1, .05, .03, .02, .01];
                    ...
                    this.linesMesh3 = new r.LineSegments(d, u), this.linesMesh3.position.set(0, 0, 0), this.cont2.add(this.linesMesh3);
                    const m = new r.LineBasicMaterial({
                            color: this.options.color
                        }),
                        f = new r.SphereGeometry(18 * this.options.size, 18, 14),
                        w = new r.EdgesGeometry(f);
                    this.sphere = new r.LineSegments(w, m), this.sphere.position.set(0, 0, 0), this.cont2.add(this.sphere), this.cont2.rotation.x = -.25
                }
                onUpdate() {
                    let t;
                    null != this.helper && this.helper.update(), null != this.controls && this.controls.update();
                    const e = this.camera;
                    Math.abs(e.tx - e.position.x) > .01 && (t = e.tx - e.position.x, e.position.x += .02 * t), Math.abs(e.ty - e.position.y) > .01 && (t = e.ty - e.position.y, e.position.y += .02 * t), n && window.innerWidth < 480 ? e.lookAt(new r.Vector3(-10, 0, 0)) : n && window.innerWidth < 720 ? e.lookAt(new r.Vector3(-20, 0, 0)) : e.lookAt(new r.Vector3(-40, 0, 0));
                    let i = 0,
                        s = 0,
                        o = 0;
                    const h = new r.Color(this.options.backgroundColor),
                        ...
                        c = a.clone().sub(h);
                    this.rayCaster && this.rayCaster.setFromCamera(new r.Vector2(this.rcMouseX, this.rcMouseY), this.camera), this.linesMesh2 && (this.linesMesh2.rotation.z += .002, this.linesMesh2.rotation.x += 8e-4, this.linesMesh2.rotation.y += 5e-4), this.sphere && (this.sphere.rotation.y += .002, this.linesMesh3.rotation.y -= .004);
                    for (let t = 0; t < this.points.length; t++) {
                        let e, n;
                        const l = this.points[t],
                            p = (n = this.rayCaster ? this.rayCaster.ray.distanceToPoint(l.position) : 1e3).clamp(5, 15);
                        l.scale.z = (.25 * (15 - p)).clamp(1, 100), l.scale.x = l.scale.y = l.scale.z, l.position.y = 2 * Math.sin(l.position.x / 10 + .01 * this.t + l.position.z / 10 * .5);
                        for (let n = t; n < this.points.length; n++) {
                            ...
                            if ((e = Math.sqrt(p * p + u * u + d * d)) < this.options.maxDistance) {
                                let n, p = 2 * (1 - e / this.options.maxDistance);
                                p = p.clamp(0, 1), n = "additive" === this.blending ? new r.Color(0).lerp(c, p) : h.clone().lerp(a, p), this.linePositions[i++] = l.position.x, this.linePositions[i++] = l.position.y, this.linePositions[i++] = l.position.z, this.linePositions[i++] = t.position.x, this.linePositions[i++] = t.position.y, this.linePositions[i++] = t.position.z, this.lineColors[s++] = n.r, this.lineColors[s++] = n.g, this.lineColors[s++] = n.b, this.lineColors[s++] = n.r, this.lineColors[s++] = n.g, this.lineColors[s++] = n.b, o++
                            }
                        }
                    }
                ...
                onMouseMove(t, e) {
                    ...
                    const s = Math.atan2(i.oz, i.ox),
                        ...
                        n = s + 1.5 * (t - .5) * (this.options.mouseCoeffX || 1);
                    i.tz = o * Math.sin(n), i.tx = o * Math.cos(n), i.ty = i.oy + 80 * (e - .5) * (this.options.mouseCoeffY || 1), this.rayCaster, this.rcMouseX = 2 * t - 1, this.rcMouseY = 2 * -t + 1
                }
                onRestart() {
